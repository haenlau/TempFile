...
| 单文件 `> 99 MiB` 且 `<= 2 GiB` | 仅支持 WebDAV 分片上传，分片大小为 `48 MiB` |
| `> 2 GiB` | 拒绝上传 |
| 多文件上传 | 自动打包 ZIP，但仅支持总大小 `<= 99 MiB` |

WebDAV 分片上传时，KV 只保存短链索引和分片清单；文件本体会以多个 `.part0000`、`.part0001` 文件保存到 WebDAV。下载时仍然使用同一个短链接，Worker 会按顺序把 WebDAV 分片流式拼回原文件。
...
WebDAV 分片上传会在目标目录写入多个分片文件。KV 的 7 天 TTL 只会删除短链记录，不会自动删除 WebDAV 上的文件本体或分片；需要在 WebDAV 后端配置生命周期清理，或后续增加定时清理任务。
...
- WebDAV 分片下载最多约 43 个分片请求，48MiB 分片大小是为了兼顾 Cloudflare 子请求数量和 Worker 内存。
